...
address_type soln_get_set  (Cache *cache, address_type address);
address_type soln_get_line (Cache *cache, address_type address);
address_type soln_get_byte (Cache *cache, address_type address);
...
int test_read_address (CPU *cpu, TraceLine *trace_line) {
  return fscanf(cpu->address_trace, "%c %lx,%d\n",
...
void test_cat_trace () {
  ...
  test(cpu->cold+cpu->conflict == 1599,
	   ...
	   cpu->cold+cpu->conflict, 1599),
       16,
       nearness(cpu->cold+cpu->conflict, 1599, 4));

  test(cpu->hits == 297076,
       msg("the number of hits (cpu->hits) is correct (%d)", cpu->hits),
       msg("the number of hits (cpu->hits) is incorrect was %d; expected %d", 
	   cpu->hits, 297076),
       17,
       nearness(cpu->hits, 297076, 4));

  test(cpu->conflict == 1535,
...
void test_ls_trace () {
  ...
  test(cpu->cold+cpu->conflict == 272802,
       msg("the number of misses (cpu->cold+cpu->conflict) is correct (%d)", cpu->cold+cpu->conflict),
       msg("the number of misses (cpu->cold+cpu->conflict) is incorrect was %d; expected %d", 
	   cpu->cold+cpu->conflict, 272802),
       19,
       nearness(cpu->cold+cpu->conflict, 272802, 4));

  test(cpu->hits == 991220,
       msg("the number of hits (cpu->hits) is correct (%d)", cpu->hits),
       msg("the number of hits (cpu->hits) is incorrect was %d; expected %d", 
	   cpu->hits, 991220),
       20,
       nearness(cpu->hits, 991220, 4));

  test(cpu->conflict == 272786,
       msg("the number of evictions (cpu->conflict) is correct (%d)", cpu->conflict),
       msg("the number of hits (cpu->conflict) is incorrect was %d; expected %d", 
	   cpu->conflict, 272786),
       21,
       nearness(cpu->conflict, 272786, 4));

  /* float misses_p   = ((float)(cpu->cold+cpu->conflict))/((float)272802) * 4; */
  /* float hits_p     = ((float)(cpu->hits))/((float)991220) * 4; */
  /* float conflict_p = ((float)(cpu->conflict))/((float)272786) * 4; */
...
void test_wc_trace () {
  ...
  test(cpu->cold+cpu->conflict == 13382,
       msg("the number of misses (cpu->cold+cpu->conflict) is correct (%d)", cpu->cold+cpu->conflict),
       msg("the number of misses (cpu->cold+cpu->conflict) is incorrect was %d; expected %d", 
	   cpu->cold+cpu->conflict, 13382),
       22,
       nearness(cpu->cold+cpu->conflict, 13382, 4));

  test(cpu->hits == 295660,
       msg("the number of hits (cpu->hits) is correct (%d)", cpu->hits),
       msg("the number of hits (cpu->hits) is incorrect was %d; expected %d", 
	   cpu->hits, 295660),
       23,
       nearness(cpu->hits, 295660, 4));

  test(cpu->conflict == 13366,
       msg("the number of evictions (cpu->conflict) is correct (%d)", cpu->conflict),
       msg("the number of hits (cpu->conflict) is incorrect was %d; expected %d", 
	   cpu->conflict, 13366),
       24,
       nearness(cpu->conflict, 13366, 4));

  /* float misses_p   = ((float)(cpu->cold+cpu->conflict))/((float)13382) * 4; */
  /* float hits_p     = ((float)(cpu->hits))/((float)295660) * 4; */
  /* float conflict_p = ((float)(cpu->conflict))/((float)13366) * 4; */
  /* sim_test_pts += (int)(misses_p+hits_p+conflict_p); */
}
